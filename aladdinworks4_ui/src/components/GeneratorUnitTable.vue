        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Generator Units</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalGeneratorUnits = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalGeneratorUnits">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add GeneratorUnit</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="GeneratorUnitId" type="text" placeholder="Enter GeneratorUnitId" v-model="generatorUnitToAdd.generatorUnitId"></base-input>
  <base-input label="PowerOutput" type="text" placeholder="Enter PowerOutput" v-model="generatorUnitToAdd.powerOutput"></base-input>
  <base-input label="FuelType" type="text" placeholder="Enter FuelType" v-model="generatorUnitToAdd.fuelType"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="generatorUnits" :row-key="record => record.GeneratorUnitId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <GeneratorUnitPictureView :generatorUnits="generatorUnits" />
          </div>
...
import GeneratorUnitPictureView from './GeneratorUnitPictureView.vue';


const generatorUnitsColumns = [
  "generatorUnitId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    GeneratorUnitPictureView
  },

  data() {
    return {
      modalGeneratorUnits: false,
        isTableView: true,

      columns: [
        {
          title: 'Generator Unit Id',
		dataIndex: 'generatorUnitId',
          visible: true,
          scopedSlots: { customRender: 'generatorUnitId' },
          sorter: true
          //sorter: (a, b) => a.generatorUnitId - b.generatorUnitId,
          //sorter: (a, b) => a.generatorUnitId.localeCompare(b.generatorUnitId),
        },
        {
          title: 'Power Output',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} generatorUnits`,
      },
      ...
      generatorUnitsTable: {
        columns: [...generatorUnitsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'generatorUnitId',           // Column to sort by
  ...
  methods: {
    ...
    async renderGeneratorUnitsTable() {
      ...
      let generatorUnitsTableData = [];
      for (let i = 0; i < this.generatorUnits.length; i++) {
        generatorUnitsTableData.push({
          id: i,
          generatorUnitId: this.generatorUnits[i].generatorUnitId,
          year: this.generatorUnits[i].year,
          date: this.generatorUnits[i].date,
          competitionId: this.generatorUnits[i].competitionId,
          generatorUnitId: this.generatorUnits[i].generatorUnitId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-generator-units',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForGeneratorUnitsChanged', this.searchQuery);
		//this.renderGeneratorUnitsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalGeneratorUnits = false;

      const currentDate = new Date().getTime();
      this.generatorUnitToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.generatorUnitToAdd);
      console.log(jsonData);
      
      const res = await GeneratorUnitService.addGeneratorUnit(jsonData);
