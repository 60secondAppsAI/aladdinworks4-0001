					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateSwitchDevice()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2SwitchDevices = true">Add</base-button>
								              <modal :show.sync="modal2SwitchDevices">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Switch Device Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="SwitchDeviceId" type="text" placeholder="Enter SwitchDeviceId" v-model="switchDeviceDetails.switchDeviceId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Ports</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Ports" type="text" placeholder="Enter Ports" v-model="switchDeviceDetails.ports"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Bandwidth</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Bandwidth" type="text" placeholder="Enter Bandwidth" v-model="switchDeviceDetails.bandwidth"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  data() {
    return {
      switchDeviceToAdd: {},
      modal2SwitchDevices: false,
      switchDeviceDetails: null,
  ...
  methods: {
    ...
    async handleAddSubmitted() {
      this.modal2SwitchDevices = false;
    ...
    async updateSwitchDevice() {

      const jsonData = JSON.stringify(this.switchDeviceDetails);
      const res = await SwitchDeviceService.update(jsonData);
...
//        this.getAllSwitchDevices();
    ...
    async getSwitchDeviceDetails() {
      try {
			let response = await SwitchDeviceService.get(this.switchDeviceId);
			this.switchDeviceDetails = response.data;
