					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateCurrentAlert()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2CurrentAlerts = true">Add</base-button>
								              <modal :show.sync="modal2CurrentAlerts">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Current Alert Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="CurrentAlertId" type="text" placeholder="Enter CurrentAlertId" v-model="currentAlertDetails.currentAlertId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Exceeded Current</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="ExceededCurrent" type="text" placeholder="Enter ExceededCurrent" v-model="currentAlertDetails.exceededCurrent"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async handleAddSubmitted() {
      this.modal2CurrentAlerts = false;
    ...
    async updateCurrentAlert() {

      const jsonData = JSON.stringify(this.currentAlertDetails);
      const res = await CurrentAlertService.update(jsonData);
...
//        this.getAllCurrentAlerts();
    ...
    async getCurrentAlertDetails() {
      try {
			let response = await CurrentAlertService.get(this.currentAlertId);
			this.currentAlertDetails = response.data;
